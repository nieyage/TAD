...
#Day5 vs Day2
Day5_0 <-rownames(subset(results(dds,contrast = c("timepoint","Day5","Day2")),
                         padj < 0.05 & abs(log2FoldChange)> 1)) 

#plot all Day5_Day2 DEG:
targetcount<-normalized_counts[Day5_0,c(4:9,19:21)]
...
MS_gene <-rownames(subset(MS_H1_Day5,padj < 0.05 & log2FoldChange > 1))
H1_gene <-rownames(subset(MS_H1_Day5,padj < 0.05 & log2FoldChange < -1))
...
cluster1<-intersect(Day5,H1_gene)
cluster2<-intersect(Day2,MS_gene)
write.csv(cluster1,"MEIS1-Day5vsDay2-KD-down-up-down.csv")
write.csv(cluster2,"MEIS1-Day5vsDay2-KD-up-down-up.csv")

gene<-c(cluster1,cluster2)
targetcount<-normalized_counts[gene,c(4:9,19:21)]
count=t(scale(t(targetcount),scale = T,center = T))
head(count)
count<-na.omit(count)
library(pheatmap)
